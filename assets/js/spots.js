	...
	$( document ).ready( function( ) {
		...
		if ( adminpage == 'widgets-php' ) {
			...
			$( document ).ajaxComplete( function( e, h, o ) {
				var req = o !== undefined && o.data !== undefined ? o.data.split( '&' ) : [],
					data = {}, _tmp, tx, i;

				for ( i in req ) {
                    if (typeof req[i] !== 'string') continue;
				...
				if ( data.action == 'save-widget' && data.id_base == bn ) {
					...
					if (tx.length) startMCE( tx.addClass( 'mceEdit' ).attr( 'id' ) );
				}
			...
			// set active editor
			$( document ).on( 'click.focusmce', '.wp-media-buttons button', function() {
