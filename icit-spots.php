		...
		*/
		...
		function admin_head( ) {
			...
			<style type="text/css">
				...
				#adminmenu #menu-posts-spot div.wp-menu-image:before,
				...
				#adminmenu #menu-posts-spot div.wp-menu-image:before 		{ background-position: -24px 4px }
				#adminmenu #menu-posts-spot:hover div.wp-menu-image:before { background-position: -24px -24px }
		...
		function mce_popup( ) {
			...
			<html <?php language_attributes( ); ?>>
				<head>
					...
					<script src="<?php echo set_url_scheme( includes_url( '/js/tinymce/tiny_mce_popup.js' ), is_ssl() ? 'https' : 'http' ) ?>"></script>
					<script src="<?php echo set_url_scheme( includes_url( '/js/tinymce/utils/form_utils.js' ), is_ssl() ? 'https' : 'http' ) ?>"></script>
					<script src="<?php echo set_url_scheme( includes_url( '/js/jquery/jquery.js' ), is_ssl() ? 'https' : 'http' ) ?>"></script>
					<script src="<?php echo trailingslashit( SPOTS_URL );?>assets/js/jquery.autocomplete.js"></script>
					<script>var icitFinderL10 = <?php echo json_encode(admin_url( 'admin-ajax.php' )); ?> ;</script>
					<script src="<?php echo trailingslashit( SPOTS_URL );?>assets/js/finder.js"></script>
					<link rel="stylesheet" href="<?php echo trailingslashit( SPOTS_URL );?>assets/popup.css">
				</head>
	...
	class Spot_Widget extends WP_Widget {
		...
		function update( $new_instance, $old_instance ) {
			...
			if ( ( empty( $instance[ 'id' ] ) || $instance[ 'id' ] == 0 ) && ! empty( $instance[ 'title' ] ) )
				$instance[ 'id' ] = wp_insert_post( array( 'post_title' => $instance[ 'title' ], 'post_content' => '', 'post_type' => SPOTS_POST_TYPE, 'post_status' => 'publish' ) );
